...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  function buildObservableType(model, method) {
	 let modelName = model.name[0].toUpperCase() + model.name.slice(1);
    let type = 'any';
    if (
      method.name.match(/(^createMany$)/g) ||
      (
        typeof method.returns === 'object' &&
        (String(method.returns.type).toLowerCase() === 'array' || Array.isArray(method.returns.type))
      )
    ) type = `${modelName}[]`;
    if (method.name.match(/(^create$|upsert|^findBy|^findOne$)/g)) type = modelName;
    return type;
  }
  /**
